// scripts/sendBatchTest.ts
import fs from 'fs';
import path from 'path';
import nodemailer from 'nodemailer';

const EMAIL = 'contact@mzprimer.com';
const PASSWORD = process.env.EMAIL_PASSWORD || 'your_password_here';

const batchPath = path.join(process.cwd(), 'data', 'batches', 'batch-test.json');
const contacts = JSON.parse(fs.readFileSync(batchPath, 'utf-8'));

const subject = '🚀 Recover Smartly – Trade Smarter with AI Tools (MZPrimer)';

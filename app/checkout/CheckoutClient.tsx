...
const BOTS: Record<string, Bot> = {
  ...
  // botId === 'hedge'     ? 'hedge-matrix' :
  // botId === 'trendbot'  ? 'trend-seeker-ai' :
};
...
// ⬇️ paste your whole CheckoutPage component body here,
// and rename it to CheckoutClient
export default function CheckoutClient() {
  ...
  const [timeLeft, setTimeLeft] = useState<number>(30 * 60); // 10 minutes in seconds
  ...
  async function createUsdtOrder(toEmail: string) {
    // Map your UI bot id → canonical product id the server expects
    const productIdHint =
      botId === 'scalper'   ? 'scalper-x1' :
      botId === 'fibonacci' ? 'fibonacci-pro' :
      botId === 'hedge'     ? 'hedge-matrix' :
      botId === 'trendbot'  ? 'trend-seeker-ai' :
      'scalper-x1'; // default fallback

    const payload = {
      method: 'usdt' as const,
      email: toEmail,
      // send these so the API can use/validate them
  ...
  useEffect(() => {
    ...
    const t = setInterval(async () => {
      try {
        ...
        // do not flip to error on transient network issues
      }
    }, 5000);
  ...
  useEffect(() => {
    ...
    if (timerActive && timeLeft > 0) {
      ...
      // Mark order as expired on the backend
      if (orderId) {
        fetch(`/api/order/expire?orderId=${orderId}`, {
          method: 'POST'
        }).catch(console.error);
      }
    }
  ...
  const handleConfirmPayment = async () => {
    ...
    if (!email) {
      setErrMsg('Please enter your email so we can send the download link.');
      return;
    }
    try {
      ...
      setTimerActive(true); // ← ADD THIS LINE
      setTimeLeft(30 * 60); // ← ADD THIS LINE
    } catch {
        ...
        {paymentMethod === 'card' ? (
          ...
          <div className="usdt-section">
            ...
            <button
              ...
              disabled={paymentStatus === 'confirmed' || paymentStatus === 'waiting'}
              className={`confirm-btn ${paymentStatus === 'confirmed' ? 'verified' : ''} ${paymentStatus === 'waiting' ? 'waiting' : ''}`}
            >
              {paymentStatus === 'confirmed' ? 'Payment Verified'
               : paymentStatus === 'waiting' ? 'Checking Payment...'
               : 'Confirm Payment'}
            </button>

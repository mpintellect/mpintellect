...
export default function TrendClientView({ data, symbol, locale, onRefresh }: TrendClientViewProps) {
  ...
  if (!data || !data.trend) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center text-gray-900">
        <p className="text-gray-500">No trend data available for {symbol.toUpperCase()}</p>
        <button
  ...
  const accentColor = isBullish ? "text-emerald-500" : "text-rose-500";
  ...
  const report = unifiedGenerator({ data, symbol, locale, tool: 'trend' }) as any;
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900 pb-24 font-sans selection:bg-blue-500/30" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <div className="pt-28 pb-10 px-6 text-center max-w-4xl mx-auto">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 mb-6 rounded-full border border-black/10 bg-gray-50 text-gray-500 text-[10px] uppercase font-bold tracking-widest">
           <Activity size={12} className="text-blue-500" /> {t.algoTrendEngine}
        </div>
      ...
      <div className="max-w-5xl mx-auto px-4 grid md:grid-cols-12 gap-8">
        ...
        <div className={`md:col-span-8 p-8 rounded-3xl relative overflow-hidden glass-trend-card ${bgTint}`}>
            ...
            <div className="flex items-center justify-between border-b border-gray-200 pb-4 mb-8">
                ...
                <span className={`text-xs font-black uppercase px-2 py-1 rounded bg-gray-50 border border-gray-200 ${accentColor}`}>
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-10">
                ...
                <div className="relative w-64 h-64 shrink-0">
                    <svg className="w-full h-full -rotate-90" viewBox="0 0 36 36">
                        {/* Background Track */}
                        <path className="text-black/5" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="currentColor" strokeWidth="2" />
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                        <span className="text-6xl font-black tracking-tighter text-gray-900">{trendData.trend_strength_score || 50}</span>
                        <span className="text-[10px] text-gray-500 uppercase font-bold tracking-widest mt-1">/ 100 {t.strength}</span>
                    </div>
                </div>
                ...
                <div className="w-full space-y-6">
                    <div>
                        <h3 className="text-[10px] text-gray-500 font-bold uppercase mb-3 flex items-center gap-2">
                            <Layers size={12} /> {t.emaStructure}
                        </h3>
                        <div className="grid grid-cols-2 gap-3">
                            <div className="bg-gray-50 p-3 rounded-xl border border-gray-200 flex flex-col">
                                <span className="text-[9px] text-gray-500 uppercase font-bold">{t.structureMode}</span>
                                <span className="text-sm font-bold text-gray-900 capitalize mt-1">
                                    {(trendData.ema_alignment || 'neutral').replace(/_/g, ' ')}
                                </span>
                            </div>
                            <div className="bg-gray-50 p-3 rounded-xl border border-gray-200 flex flex-col">
                                <span className="text-[9px] text-gray-500 uppercase font-bold">{t.position}</span>
                    ...
                    <div className="space-y-2">
                        <div className="flex justify-between items-center bg-gray-50 p-2 px-3 rounded-lg border border-gray-200">
                            <span className="text-xs font-medium text-emerald-500">{t.emaFast}</span>
                            <span className="text-sm font-mono text-gray-900 font-bold">{fmt(trendData.current_emas?.ema_8)}</span>
                        </div>
                        <div className="flex justify-between items-center bg-gray-50 p-2 px-3 rounded-lg border border-gray-200">
                            <span className="text-xs font-medium text-blue-500">{t.emaBaseline}</span>
                            <span className="text-sm font-mono text-gray-900 font-bold">{fmt(trendData.current_emas?.ema_21)}</span>
                        </div>
                        <div className="flex justify-between items-center bg-gray-50 p-2 px-3 rounded-lg border border-gray-200">
                            <span className="text-xs font-medium text-blue-600">{t.emaMacro}</span>
                            <span className="text-sm font-mono text-gray-900 font-bold">{fmt(trendData.current_emas?.ema_50)}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div className="md:col-span-4 flex flex-col gap-6">
            <div className="p-6 rounded-3xl border border-gray-200 bg-gray-50 flex flex-col items-center justify-center text-center h-full">
                ...
                <h4 className="text-gray-900 font-bold text-lg mb-2">{t.trackSymbol} {resolvedSymbol}</h4>

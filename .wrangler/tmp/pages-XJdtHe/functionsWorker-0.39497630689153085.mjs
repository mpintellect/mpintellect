...
var init_utils = __esm({
  "../node_modules/unenv/dist/runtime/_internal/utils.mjs"() {
    init_functionsRoutes_0_8546667375405167();
...
async function onRequestPost(context2) {
  ...
  try {
    ...
    try {
      ...
      `).run();
      const sessionResult = await env2.DB.prepare(
    ...
    return new Response(
      JSON.stringify({
        ...
        debug: true ? error3.message : void 0
      }),
      {
        status: 500,
...
async function onRequestGet(context2) {
  ...
  try {
    ...
    console.log(`\u{1F50D} [ME] Checking Token: ${token.substring(0, 8)}...`);
    ...
    console.error("\u{1F4A5} [ME] Crash:", error3.message);
    return new Response(
...
async function onRequestPost3(context2) {
  ...
  try {
    ...
    const token = crypto.randomUUID();
    const now = (/* @__PURE__ */ new Date()).toISOString().replace("T", " ").replace("Z", "");
    const expiresAt = new Date(Date.now() + 30 * 24 * 60 * 60 * 1e3).toISOString().replace("T", " ").replace("Z", "");
    console.log(`\u{1F4DD} [REG] Registering user: ${cleanEmail}`);
    console.log(`\u{1F3AB} [REG] Creating Token: ${token.substring(0, 8)}...`);
    await env2.DB.prepare(
    ...
    console.error("\u{1F4A5} [REG] Crash:", error3.message);
    return new Response(
...
async function onRequestPost11(context2) {
  ...
  try {
    ...
    const origin = env2.NEXT_PUBLIC_SITE_URL || url.origin;
    console.log(`\u{1F4B0} Creating Stripe session for User: ${userId}, Plan: ${plan}`);
    const session = await stripe.checkout.sessions.create({
      payment_method_types: ["card"],
      mode: "payment",
      line_items: [{ price: priceId, quantity: 1 }],
    ...
    console.error("\u274C Stripe Production Error:", error3.message);
...
async function onRequestGet2(context2) {
  ...
  try {
    ...
    const referralResult = await env2.DB.prepare(
      ...
      if (authHeader) {
        ...
        if (authHeader.startsWith("Bearer ")) {
          ...
          try {
            ...
            console.error("Failed to decode cf_ token:", error3);
          }
        }
      }
    }
...
async function onRequestGet4(context2) {
  ...
  try {
    ...
    console.error("\u274C Trial status error:", error3);
    return Response.json({
      success: false,
...
async function onRequestPost13(context2) {
  ...
  try {
    ...
    console.error("\u274C Increment trial error:", error3);
    return Response.json({
      success: false,
      error: error3.message
    }, { status: 500 });
...
async function onRequestPost15(context2) {
  ...
  try {
    ...
    try {
      ...
      console.error("\u274C Failed to parse request body:", error3);
    }
...
async function onRequestPost16(context2) {
  ...
  const body = await request.text();
  try {
    const event = stripe.webhooks.constructEvent(body, sig, env2.STRIPE_WEBHOOK_SECRET_CHATBOT);
    if (event.type === "checkout.session.completed") {
      const session = event.data.object;
      ...
      if (userId && setupsToAdd) {
        console.log(`\u{1F389} Success: Adding ${setupsToAdd} setups to user ${userId}`);
        await execute(
...
async function onRequest(context2) {
  ...
  try {
    ...
    console.error("\u274C Validation error:", error3);
    return new Response(
      JSON.stringify({ success: false, error: error3.message }),
      { status: 500, headers: { "Content-Type": "application/json" } }
    );
...
async function onRequestPost18(context2) {
  ...
  try {
    ...
    for (const sql of tables) {
      ...
      const result = await env2.DB.prepare(sql).run();
      results.push({ sql, success: result.success });
    }
    const tablesCheck = await env2.DB.prepare(
      "SELECT name FROM sqlite_master WHERE type='table' ORDER BY name"
    ).all();
    const tableNames = tablesCheck.results?.map((t) => t.name) || [];
    console.log("\u2705 Database initialized successfully!");
    console.log("\u{1F4CA} Tables created:", tableNames);
    return new Response(JSON.stringify({
      success: true,
    ...
    console.error("\u{1F4A5} Database initialization failed:", error3.message);
    console.error("\u{1F4A5} Error stack:", error3.stack);
    return new Response(JSON.stringify({
      success: false,
      error: error3.message,
      stack: true ? error3.stack : void 0
    }), {
      status: 500,
      headers: {
        "Content-Type": "application/json",
...
async function onRequestGet6(context2) {
  const { env: env2 } = context2;
  try {
    const tables = await env2.DB.prepare(
      "SELECT name FROM sqlite_master WHERE type='table' ORDER BY name"
    ).all();
    return new Response(JSON.stringify({
      success: true,
    ...
    console.error("\u{1F4A5} Failed to check tables:", error3.message);
    return new Response(JSON.stringify({
      success: false,
      error: error3.message,
...
async function onRequestGet7(context2) {
  ...
  try {
    ...
    console.error("Error in livemarketfeed function:", error3);
    return new Response(
      JSON.stringify({
...
async function onRequestGet8() {
  try {
    ...
    console.error("News API Error:", error3);
    return new Response(JSON.stringify([]), {
      status: 200,
      headers: {
        "Content-Type": "application/json",
...
// ../.wrangler/tmp/pages-XJdtHe/functionsRoutes-0.8546667375405167.mjs
var routes;
var init_functionsRoutes_0_8546667375405167 = __esm({
  "../.wrangler/tmp/pages-XJdtHe/functionsRoutes-0.8546667375405167.mjs"() {
    "use strict";
    init_login();
    init_logout();
...
// ../.wrangler/tmp/bundle-GqCXR5/middleware-loader.entry.ts
init_functionsRoutes_0_8546667375405167();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();

// ../.wrangler/tmp/bundle-GqCXR5/middleware-insertion-facade.js
init_functionsRoutes_0_8546667375405167();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
...
//# sourceMappingURL=functionsWorker-0.39497630689153085.mjs.map

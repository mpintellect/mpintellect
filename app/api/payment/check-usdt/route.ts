...
import { getOrder, updateOrder, issueDownloadToken, isTransactionAlreadyUsed } from "../../../lib/orders";
...
export async function GET(req: Request) {
  ...
  if (!orderId) return NextResponse.json({ status: "pending", error: "orderId missing" }, { status: 200 });

  const order = getOrder(orderId);
  if (!order) return NextResponse.json({ status: "pending", error: "Order not found" }, { status: 200 });
  if (order.method !== "usdt") return NextResponse.json({ status: "pending", error: "Wrong method" }, { status: 200 });

  // Already paid → reissue a fresh one-time token (idempotent)
  if (order.status === "paid") {
    const { rawToken, expiresAt } = issueDownloadToken(order.id, 24 * 3600);
    return NextResponse.json({ status: "paid", token: rawToken, expiresAt }, { status: 200 });
  }

  if (!TRON_API_KEY || !WALLET) {
    return NextResponse.json({ status: "pending", error: "Server missing Tron env" }, { status: 200 });
  }
try {
  ...
  if (!res.ok) {
    ...
    return NextResponse.json({ status: "pending", error: `TronGrid ${res.status}` }, { status: 200 });
  }
  ...
  if (transfers.length === 0) {
    ...
    console.log('📋 API RESPONSE SAMPLE (first transaction):', transfers[0] ? {
...
transfers.forEach((tx: any, index: number) => {
  ...
  const timestamp = tx?.block_timestamp ? new Date(Number(tx.block_timestamp)).toISOString() : 'unknown';
...
const match = transfers.find((tx: any) => {
  try {
    ...
    const okContract = (tx?.token_info?.address || USDT_CONTRACT) === USDT_CONTRACT;
    ...
    console.log('🔍 RELAXED SECURITY CHECK:', {
      ...
      okContract,
      okType,
      transactionTime: new Date(tsMs).toISOString(),
...
if (!match) {
  ...
  console.log('- Any transactions to our wallet?', transfers.some((tx: any) => (tx?.to || "").toLowerCase() === WALLET.toLowerCase()));
  console.log('- Any transactions with correct amount?', transfers.some((tx: any) => Math.abs((Number(tx?.value) / 1e6) - order.amountUsd) < 0.001));
  console.log('- Any incoming payments?', transfers.some((tx: any) => (tx?.to || "").toLowerCase() === WALLET.toLowerCase()));
  console.log('- Order amount expected:', order.amountUsd);
  
  // Check if we're even getting any transactions
  if (transfers.length === 0) {
    console.log('⚠️  NO transactions returned from TronGrid API - check API key or wallet');
  }
...
console.log('New status:', paid?.status);
    ...
    if (paid?.email) {
      ...
      console.warn("⚠️ [check-usdt] No email on order; skipping send", { orderId: order.id });
    }
  ...
  if (e.name === 'AbortError') {
    ...
    return NextResponse.json({ status: "pending", error: "API timeout" }, { status: 200 });
  }
  console.error('💥 PAYMENT CHECK ERROR:', e.message);
  console.error('   - Error name:', e.name);
  return NextResponse.json({ status: "pending", error: e.message }, { status: 200 });
}
}
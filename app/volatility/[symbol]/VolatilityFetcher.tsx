...
import VolatilityClientView from '@/components/VolatilityClientView';
import { SymbolData } from '@/app/lib/fetchData';

export default function VolatilityFetcher({ symbol }: { symbol: string }) {
  ...
  const fetchSymbolData = async () => {
    try {
      ...
      console.log(`[VolatilityFetcher] Fetching ${cleanSymbol} from /api/symbol-data`);
      ...
      setData(result);
      console.log(`[VolatilityFetcher] Data loaded for ${cleanSymbol}`);
      
    } catch (err) {
      console.error(`[VolatilityFetcher] Error:`, err);

...
export default function UnifiedFetcher({ symbol, locale, tool }: UnifiedFetcherProps) {
  ...
  const fetchSymbolData = async () => {
    try {
      ...
      // Relative URL: Next.js rewrites proxy /api/* → production API in dev,
      // and Cloudflare Pages serves /api/* directly in production.
      const response = await fetch(`/api/symbol-data?symbol=${apiSymbol}`);
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <h2 className="text-xl text-gray-900 mb-2">{errorTitle}</h2>
  ...
  if (!data) {
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          <p className="text-gray-500">No data available for {displaySymbol.toUpperCase()}</p>
          <button
            onClick={fetchSymbolData}

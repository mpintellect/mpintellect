...
export default function VolatilityClientView({ data, symbol, locale, onRefresh }: VolatilityClientViewProps) {
  ...
  if (!data || !data.volatility) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center text-gray-900">
        <p className="text-gray-500">No volatility data available for {symbol.toUpperCase()}</p>
        <button
  ...
  const accentColor = isHighRisk ? "text-amber-500" : "text-blue-500";
  ...
  const report = unifiedGenerator({ data, symbol, locale, tool: 'volatility' }) as any;
      ...
      <div className="max-w-5xl mx-auto px-4 grid md:grid-cols-12 gap-8">
        ...
        <div className={`md:col-span-8 p-8 rounded-3xl relative overflow-hidden glass-volatility-card ${glowStyle}`}>
            ...
            <div className="flex items-center justify-between border-b border-gray-200 pb-4 mb-8">
                ...
                <span className="text-xs font-mono font-bold text-gray-900 px-2 py-1 rounded bg-gray-100">
            ...
            <div className="flex flex-col gap-8">
                ...
                <div>
                    <div className="flex justify-between text-sm font-bold text-gray-500 mb-2 uppercase text-[10px] tracking-wider">
                        <span>{t.compressedSafe}</span>
                        <span>{t.explosiveRisk}</span>
                    </div>

                    <div className="w-full h-6 bg-gray-100 rounded-full border border-gray-200 relative overflow-hidden">
                        <div className="absolute top-0 bottom-0 left-1/4 w-px bg-black/5"></div>
                        <div className="absolute top-0 bottom-0 left-2/4 w-px bg-black/5"></div>
                        <div className="absolute top-0 bottom-0 left-3/4 w-px bg-black/5"></div>

                        <div
                            className={`h-full ${barColor} relative vol-bar-glow`}
                            style={{ width: `${Math.min((v.volatility_score || 0) * 100, 100)}%` }}
                        >
                            <div className="absolute right-0 top-0 bottom-0 w-1 bg-white opacity-70 shadow-[0_0_10px_white]"></div>
                        </div>
                    </div>

                    <div className="mt-2 text-right">
                        <span className={`text-4xl font-black ${accentColor}`}>{(v.volatility_score || 0).toFixed(2)}</span>
                        <span className="text-sm text-gray-500 font-medium"> / 1.0 Index</span>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="bg-gray-50 border border-gray-200 p-4 rounded-2xl flex items-center gap-4">
                        ...
                        <div>
                            <p className="text-[10px] uppercase text-gray-500 font-bold mb-1">{t.avgRangeCapability}</p>
                            <p className="text-xl font-mono font-bold text-gray-900">{Math.floor(v.avg_range || 0)} <span className="text-xs font-sans font-normal opacity-50">PTS</span></p>
                        </div>
                    </div>
                    ...
                    <div className="bg-gray-50 border border-gray-200 p-4 rounded-2xl flex items-center gap-4">
                        ...
                        <div>
                            <p className="text-[10px] uppercase text-gray-500 font-bold mb-1">{t.recommendedStop}</p>
                            <p className="text-xl font-mono font-bold text-gray-900">{(v.optimal_sl_multiplier || 1.5)}x <span className="text-xs font-sans font-normal opacity-50">ATR</span></p>
                        </div>
                    </div>
        ...
        <div className="md:col-span-4 flex flex-col gap-6">
            <div className="p-6 rounded-3xl border border-gray-200 bg-gray-50 flex flex-col items-center justify-center text-center h-full">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-4 ${isHighRisk ? 'bg-amber-100 text-amber-600' : 'bg-blue-100 text-blue-600'}`}>
                    <Activity size={24} />
                </div>
                <h4 className="text-gray-900 font-bold text-lg mb-2">{t.regime}: {(v.volatility_regime || 'NEUTRAL').toUpperCase()}</h4>

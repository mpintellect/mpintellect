...
// .wrangler/tmp/pages-RgGITl/functionsWorker-0.4924837435608489.mjs
...
async function onRequestPost2(context22) {
  ...
  try {
    ...
    if (sessionId) {
      await env22.DB.prepare("DELETE FROM sessions WHERE id = ?").bind(sessionId).run();
    }
...
async function onRequestGet(context22) {
  ...
  try {
    ...
    console.error("\u{1F4A5} [ME] Crash:", error32.message);
    return new Response(
...
async function onRequestPost3(context22) {
  ...
  try {
    ...
    console.error("\u{1F4A5} [REG] Crash:", error32.message);
    return new Response(
...
async function onRequestPost11(context22) {
  ...
  try {
    ...
    const origin = env22.NEXT_PUBLIC_SITE_URL || url.origin;
    ...
    console.error("\u274C Stripe Production Error:", error32.message);
...
async function onRequestPost16(context22) {
  const { request, env: env22 } = context22;
  const stripe = new stripe_esm_worker_default(env22.STRIPE_SECRET_KEY, {
    // @ts-ignore
    apiVersion: "2023-10-16"
  });
  const sig = request.headers.get("stripe-signature") || "";
  const body = await request.text();
  try {
    const event = stripe.webhooks.constructEvent(body, sig, env22.STRIPE_WEBHOOK_SECRET_CHATBOT);
    if (event.type === "checkout.session.completed") {
      const session = event.data.object;
...
async function onRequestPost18(context22) {
  ...
  try {
    ...
    for (const sql of tables) {
      ...
      const result = await env22.DB.prepare(sql).run();
      results.push({ sql, success: result.success });
    }
    const tablesCheck = await env22.DB.prepare(
    ...
    console.error("\u{1F4A5} Database initialization failed:", error32.message);
    console.error("\u{1F4A5} Error stack:", error32.stack);
    return new Response(JSON.stringify({
      success: false,
      error: error32.message,
      stack: true ? error32.stack : void 0
    }), {
      status: 500,
      headers: {
        "Content-Type": "application/json",
...
async function onRequestGet6(context22) {
  const { env: env22 } = context22;
  try {
    const tables = await env22.DB.prepare(
      "SELECT name FROM sqlite_master WHERE type='table' ORDER BY name"
    ).all();
    return new Response(JSON.stringify({
      success: true,
    ...
    console.error("\u{1F4A5} Failed to check tables:", error32.message);
    return new Response(JSON.stringify({
      success: false,
      error: error32.message,
...
// .wrangler/tmp/pages-RgGITl/9kmtxgjnrz5.js
...
// .wrangler/tmp/bundle-4micM9/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-4micM9/middleware-loader.entry.ts
...
//# sourceMappingURL=9kmtxgjnrz5.js.map

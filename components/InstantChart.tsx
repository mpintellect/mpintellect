        ...
        <div className="pricing-options">
          {PRICING_PLANS.map((plan) => (
            <div key={plan.id} className={`pricing-card ${plan.popular ? "popular" : ""}`}>
              ...
              <button onClick={() => onPlanSelect(plan.id)} className="select-plan-btn">
                Select Plan
              </button>
            </div>
          ))}
...
export default function InstantChart() {
  ...
  useEffect(() => {
    ...
    const savedStrategy = localStorage.getItem(STORAGE_KEYS.STRATEGY) as Strategy;
  ...
  useEffect(() => {
    ...
    const entryPrice = pendingOrder.entry_price || chartData.chart?.current_price || 0;
    const stopLoss = pendingOrder.stop_loss || chartData.tp_sl?.sl_level || 0;
    const takeProfit = pendingOrder.take_profit || chartData.tp_sl?.tp_level || 0;
    ...
    if (entryPrice && stopLoss) {
      ...
      const calculatedLotSize = Math.max(0.01, Math.min(50, parseFloat(rawLotSize.toFixed(2))));
  ...
  useEffect(() => {
    if (!chartContainerRef.current || !chartData?.candles?.data || chartData.candles.data.length === 0) {
      return;
    }
    ...
    if (chartData.pivot?.level) {
      const pivotLine = chart.addSeries(LineSeries, { color: "#D4AF37", lineWidth: 2, lineStyle: 2 });
      pivotLine.setData([
    ...
    if (chartData.pivot?.resistance_1) {
      const r1Line = chart.addSeries(LineSeries, { color: "#EF4444", lineWidth: 2, lineStyle: 2 });
      r1Line.setData([
    ...
    if (chartData.pivot?.support_1) {
      const s1Line = chart.addSeries(LineSeries, { color: "#10B981", lineWidth: 2, lineStyle: 2 });
      s1Line.setData([
    ...
    const currentPrice = chartData.chart?.current_price || chartData.pending_orders?.current_price;
    if (currentPrice) {
      const currentLine = chart.addSeries(LineSeries, { color: "#D4AF37", lineWidth: 2, lineStyle: 2 });
      currentLine.setData([
    ...
    if (tp) {
      const tpLine = chart.addSeries(LineSeries, { color: "#10B981", lineWidth: 1, lineStyle: 2 });
      tpLine.setData([
    ...
    if (sl) {
      const slLine = chart.addSeries(LineSeries, { color: "#EF4444", lineWidth: 1, lineStyle: 2 });
      slLine.setData([
    ...
    const handleResize = () => {
      if (chartContainerRef.current && chartRef.current) {
        try { chartRef.current.applyOptions({ width: chartContainerRef.current.clientWidth }); } catch (e) { }
      }
    };
  ...
  const handlePlanSelect = async (planId: string) => {
    ...
    if (user) {
      const response = await fetch("/api/checkout/create-session", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ userId: user.id, plan: planId, email: user.email }),
      });
      const data = await response.json();
      ...
      <div className="paywall-container">
        ...
        <div className="license-options">
          <div className="license-option">
            ...
            <button type="button" onClick={() => setShowPricingModal(true)} className="btn-gold">
              View Plans
            </button>
          </div>
          ...
          {!user && (
            <div className="license-option">
              ...
              <button type="button" onClick={() => { setShowPricingModal(false); setShowRegisterModal(true); }} className="btn-ghost-gold">
                Register Now
              </button>
            </div>
          )}
        ...
        {showPricingModal && (
          <PricingPlansModal
            ...
            onRegisterClick={() => { setShowPricingModal(false); setShowRegisterModal(true); }}
          />
        )}
        {showRegisterModal && (
          <QuickRegisterModal
  ...
  return (
    <>
      ...
      <div className="chart-header">
        <h1>📊 INSTANT TECHNICAL ANALYSIS</h1>
        <div className="subtitle">Professional Charting & Analysis Tools</div>
      </div>

      {/* Credits Bar */}
      <div className="credits-bar">
        <span>💳 Credits remaining: {creditsRemaining}</span>
        <button onClick={() => setShowPricingModal(true)} className="buy-credits-btn">
          Buy More
        </button>
      </div>
          ...
          <div className="strategy-buttons">
            {STRATEGY_OPTIONS.map((opt) => (
              <button
                ...
                className={`strategy-btn ${strategy === opt.id ? "active" : ""}`}
              >
                <span>{opt.icon}</span>
                <span>{opt.name}</span>
                <span className="timeframe">{opt.timeframe}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="capital-risk-section">
          ...
          <div className="risk-presets">
            {RISK_PRESETS.map((risk) => (
              <button
                ...
                className={`risk-preset-btn ${riskPercent === risk ? "active" : ""}`}
              >
                {risk}%
              </button>
            ))}
      ...
      {/* Analyze Button */}
      <button onClick={fetchChartData} disabled={loading} className="analyze-btn">
      ...
      {loading && !chartData && (
        <div className="loading-container">
          ...
          <p>Fetching market data & generating analysis...</p>
        </div>
      )}
        ...
        <>
          ...
          <div className="lot-size-display">
            ...
            <div className="display-card">
              ...
              <div className="value" style={{ color: "#10B981" }}>${rewardAmount.toFixed(2)}</div>
            </div>
            <div className="display-card">
      ...
      {!chartData && !loading && !error && (
        <div className="no-data-container">
          <p>📊 Click "ANALYZE NOW" to view the chart and trading analysis</p>
        </div>
      )}

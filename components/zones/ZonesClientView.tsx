...
export default function ZonesClientView({ data, symbol, locale, onRefresh }: ZonesClientViewProps) {
  ...
  if (!data || !data.zones) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center text-gray-900">
        <p className="text-gray-500">No zone data available for {symbol.toUpperCase()}</p>
        <button
  ...
  const report = unifiedGenerator({ data, symbol, locale, tool: 'zones' }) as any;
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900 pb-24 font-sans selection:bg-orange-500/30" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <div className="pt-28 pb-10 px-6 text-center max-w-4xl mx-auto">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 mb-6 rounded-full border border-orange-500/30 bg-orange-50 text-orange-600 text-[10px] uppercase font-bold tracking-widest">
        ...
        <h1 className="text-4xl md:text-6xl font-black mb-4 uppercase tracking-tighter text-gray-900">
          {resolvedSymbol} <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-amber-500">{t.zones}</span>
        </h1>
      ...
      <div className="max-w-5xl mx-auto px-4 grid md:grid-cols-12 gap-8">
        ...
        {/* MAIN GLASS CARD */}
        <div className="md:col-span-8 p-8 rounded-3xl relative overflow-hidden glass-trend-card bg-gradient-to-b from-gray-50 to-white">
            ...
            <div className="flex items-center justify-between border-b border-gray-200 pb-4 mb-10">
                ...
                <span className="text-[10px] font-bold text-orange-600 uppercase tracking-widest px-2 py-1 rounded bg-orange-50 border border-orange-200">
            ...
            <div className="flex flex-col gap-12">
                ...
                <div className="flex items-stretch gap-6 h-64 md:h-80">
                    ...
                    {/* The Scale */}
                    <div className="flex flex-col justify-between text-xs font-mono text-gray-500 py-1">
                        <div className="flex items-center gap-2">
                            <span className="text-red-500 font-bold">{formatNumber(zonesData.resistance_zone)}</span>
                            <span>{t.res}</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <span className="text-emerald-500 font-bold">{formatNumber(zonesData.support_zone)}</span>
                            <span>{t.sup}</span>
                        </div>
                    </div>

                    {/* The Bar */}
                    <div className="flex-1 bg-gray-50 rounded-2xl border border-gray-200 relative w-12 md:w-20 mx-auto">

                        {/* Red Zone (Top) */}
                        <div className="absolute top-0 w-full h-8 bg-gradient-to-b from-red-100 to-transparent border-t-2 border-red-400 rounded-t-xl"></div>

                        {/* Green Zone (Bottom) */}
                        <div className="absolute bottom-0 w-full h-8 bg-gradient-to-t from-emerald-100 to-transparent border-b-2 border-emerald-400 rounded-b-xl"></div>

                        {/* PRICE MARKER (Moves dynamically) */}
                        <div
                            className="absolute left-0 right-0 h-0.5 bg-gray-900 shadow-[0_0_15px_rgba(0,0,0,0.3)] flex items-center transition-all duration-1000"
                            style={{ bottom: `${pricePct}%` }}
                        >
                            <div className="absolute w-full border-t border-dashed border-gray-400"></div>
                            <div className="absolute right-[-80px] bg-gray-900 text-white font-bold text-[10px] px-2 py-1 rounded flex items-center gap-1 shadow-lg">
                    ...
                    <div className="flex flex-col justify-center gap-4 text-xs">
                        <div className="p-3 bg-gray-50 border border-gray-200 rounded-lg">
                            <span className="block text-gray-500 uppercase text-[9px] font-bold mb-1">{t.status}</span>
                            <span className="block text-gray-900 font-bold capitalize">
                                {zonesData.current_price_position?.replace(/_/g, ' ') || 'Unknown'}
                            </span>
                        </div>
                        <div className="p-3 bg-gray-50 border border-gray-200 rounded-lg">
                            <span className="block text-gray-500 uppercase text-[9px] font-bold mb-1">{t.spread}</span>
                            <span className="block text-gray-900 font-bold">{zonesData.zone_width_pips || 0} Pips</span>
                        </div>
                    </div>
        ...
        <div className="md:col-span-4 flex flex-col gap-6">
            <div className="p-6 rounded-3xl border border-gray-200 bg-gray-50 flex flex-col items-center justify-center text-center h-full">
                <div className="w-14 h-14 rounded-full flex items-center justify-center mb-4 bg-orange-50 text-orange-600 border border-orange-200">
                    <Minimize2 size={24} />
                </div>
